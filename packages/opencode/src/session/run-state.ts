...
const cancelBackgroundJobs = Effect.fn("SessionRunState.cancelBackgroundJobs")(function* (
  ...
  const matches = (job: BackgroundJob.Info) => {
    ...
    // since cancel != exit-note). Spare ONLY those armed in the directly-aborted session:
    // an ephemeral monitor armed inside a subagent (different sessionId) is still part of
    // the aborted work and IS cancelled. All of them are still reaped on session DELETE.
    if (
      job.metadata?.background === true &&
      (job.type === "monitor" || job.type === "bash_background") &&
